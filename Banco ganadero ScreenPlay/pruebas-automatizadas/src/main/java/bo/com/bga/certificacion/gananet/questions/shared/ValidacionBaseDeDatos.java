package bo.com.bga.certificacion.gananet.questions.shared;

import net.serenitybdd.screenplay.Actor;
import net.serenitybdd.screenplay.Question;


import java.util.Map;

public class ValidacionBaseDeDatos implements Question<Boolean> {
    public String consultaBD;
    public String mensaje;
    public String key;

    public ValidacionBaseDeDatos(String mensaje,String consultaBD,String key) {
        this.mensaje = mensaje;
        this.consultaBD = consultaBD;
        this.key = key;
    }

    public Boolean answeredBy(Actor actor) {
        Map<String, Object> recall = actor.recall(key);
        if (recall == null) {
            throw new AssertionError("El registro no se almacenó en base de datos");
        }
        String consulta =  recall.get(consultaBD).toString();
        System.out.println(consulta);
        boolean resultado =consulta.equalsIgnoreCase(mensaje);
        
        if (!resultado) {
            throw new AssertionError("Ocurrió un error validando en base de dato:s el mensaje recibido no coincide con el esperado.\n" +
                    "Mensaje esperado: " + mensaje + "\n" +
                    "Mensaje recibido: " + consulta);
        }
        return resultado;
    }
    public static  ValidacionBaseDeDatos esCorrecta(String mensaje,String consultaBD,String key){
        return new ValidacionBaseDeDatos(mensaje,consultaBD,key);
    }
}

