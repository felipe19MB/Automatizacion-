...
public class ServicioValidarClave implements Interaction {
    ...
    public <T extends Actor> void performAs(T actor) {
        try {
            SerenityRest.reset();
            actor.attemptsTo(
                    Post.to(Endpoints.URL_MTDVALIDARCLAVE4_N)
                            .with(
                                    resource ->
                                       resource.header(
            ...
            if (SerenityRest.lastResponse().statusCode() != HttpStatus.SC_OK){
                ...
                String response = SerenityRest.lastResponse().path("mtdValidarClave4Response[0].strMensaje");
                String idsesion = SerenityRest.lastResponse().path("mtdValidarClave4Response[0].strNroSesion");
                actor.remember("ValidarClaveStrMensaje", response);
                actor.remember("NumeroSesion", idsesion);
            }
