...
public class DiligenciarNDestinatario implements Interaction {


    private TransferenciaModel transferenciaModel;

    public DiligenciarNDestinatario(TransferenciaModel transferenciaModel){
        this.transferenciaModel = transferenciaModel;
    }

    @Title("Se diligencia el nuevo destinatario del mismo banco")
    @Override
    public <T extends Actor> void performAs(T actor) {
        try {
            ...
            Logger.getAnonymousLogger().info("Formulario nuevo destinatario mismo banco completado");
        }catch (RuntimeException ex){
            throw new Excepciones(Excepciones.error(Interaccion.INTERACTION_FORMULARIO_NUEVO_DESTINATARIO_MISMO_BANCO), ex);
        }

    }
    public static DiligenciarNDestinatario diligenciar(TransferenciaModel transferenciaModel){
        return Instrumented.instanceOf(DiligenciarNDestinatario.class).withProperties(transferenciaModel);
    }
}
