...
public class DiligenciarOtrosBancos implements Interaction {

    private TransferenciaModel transferenciaModel;

    public DiligenciarOtrosBancos(TransferenciaModel transferenciaModel){
        this.transferenciaModel = transferenciaModel;
    }

    @Title("Se diligencia la transacción a otros bancos")
    @Override
    public <T extends Actor> void performAs(T actor) {
        try {
            actor.attemptsTo(
                    ...
                    Tap.on(IMG_BUSCAR),
                    Check.whether(isiOS(actor)).andIfSo(Tap.on(BTN_CONTACTO_IOS.of(transferenciaModel.getNombredestinatario()))
                    ).otherwise(Tap.on(BTN_CONTACTO.of(transferenciaModel.getNombredestinatario()))),
            ...
            Logger.getAnonymousLogger().info("Formulario otros bancos completado");
        } catch (RuntimeException ex) {
            throw new Excepciones(Excepciones.error(Interaccion.INTERACTION_FORMULARIO_OTROS_BANCOS), ex);
        }
    }

    public static DiligenciarOtrosBancos diligenciar(TransferenciaModel transferenciaModel){
        return Instrumented.instanceOf(DiligenciarOtrosBancos.class).withProperties(transferenciaModel);
    }
}
