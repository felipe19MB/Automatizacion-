...
public class DiligenciarTerceros implements Interaction {

    private TransferenciaModel transferenciaModel;

    public DiligenciarTerceros(TransferenciaModel transferenciaModel) {
    ...
    public <T extends Actor> void performAs(T actor) {
        try {
            ...
            Logger.getAnonymousLogger().info("Formulario terceros completado");
        } catch (RuntimeException ex) {
            throw new Excepciones(Excepciones.error(Interaccion.INTERACTION_FORMULARIO_TERCEROS), ex);
        }

    }

    public static DiligenciarTerceros diligenciar(TransferenciaModel transferenciaModel) {
        return Instrumented.instanceOf(DiligenciarTerceros.class).withProperties(transferenciaModel);
    }
}
